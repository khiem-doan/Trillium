...
def fuzzypanda():
    while True:
        try:
            ...
            soup = BeautifulSoup(resp.content, 'html.parser')
            # Find the first post
            post = soup.find('div', class_='post-item')
            if not post:
                print("FuzzyPanda: Could not find 'div' with class 'post-item'.")
                time.sleep(3)
                continue
            # Find the link to the post
            a_tag = post.find('a', href=True)
            if not a_tag:
                print("FuzzyPanda: Could not find 'a' tag with href.")
                time.sleep(3)
                continue
            article_url = a_tag['href']
            # Get the headline
            ...
            if not title_tag:
                ...
                time.sleep(3)
                continue
            fuzzypanda_headline[1] = title_tag.get_text(strip=True)
            now = datetime.now().time()
...
def scorpion():
    while True:
        try:
            ...
            resp = requests.get("https://www.scorpioncapital.com/research", headers=headers)
            if resp.status_code != 200:
            ...
            report = soup.find('div', class_='w-dyn-item')
            if not report:
                print("Scorpion: Could not find 'div' with class 'w-dyn-item'.")
                time.sleep(3)
                continue
            # Find the link to the report
            a_tag = report.find('a', href=True)
            if not a_tag:
                print("Scorpion: Could not find 'a' tag with href.")
                time.sleep(3)
                continue
            article_url = a_tag['href']
            if not article_url.startswith('http'):
                article_url = 'https://www.scorpioncapital.com' + article_url
            # Get the headline
            title_tag = report.find('div', class_='text-block-3')
            if not title_tag:
                print("Scorpion: Could not find 'div' with class 'text-block-3'.")
                time.sleep(3)
                continue
            scorpion_headline[1] = title_tag.get_text(strip=True)
            now = datetime.now().time()
...
def kerrisdale():
    while True:
        try:
            ...
            resp = requests.get("https://www.kerrisdalecap.com/blog/", headers=headers)
            if resp.status_code != 200:
                print(f"Kerrisdale: Received status code {resp.status_code}")
                time.sleep(3)
                continue
            soup = BeautifulSoup(resp.content, 'html.parser')
            # Find the latest blog post
            post = soup.find('div', class_='each-post')
            if not post:
            ...
            a_tag = post.find('a', href=True)
            if not a_tag:
                print("Kerrisdale: Could not find 'a' tag with href.")
                time.sleep(3)
                continue
            article_url = a_tag['href']
            if not article_url.startswith('http'):
                article_url = 'https://www.kerrisdalecap.com' + article_url
            # Get the headline
            title_tag = post.find('h2')
            if not title_tag:
                print("Kerrisdale: Could not find 'h2' tag.")
                time.sleep(3)
                continue
            kerrisdale_headline[1] = title_tag.get_text(strip=True)
            now = datetime.now().time()
...
def spruce():
    while True:
        try:
            ...
            resp = requests.get("https://www.sprucepointcap.com/research", headers=headers)
            if resp.status_code != 200:
                print(f"Spruce: Received status code {resp.status_code}")
                time.sleep(3)
                continue
            soup = BeautifulSoup(resp.content, 'html.parser')
            # Find the first research item
            research_item = soup.find('div', class_='research-list-wrap')
            if not research_item:
                print("Spruce: Could not find 'div' with class 'research-list-wrap'.")
                time.sleep(3)
                continue
            # Find the link to the research report
            a_tag = research_item.find('a', href=True)
            if not a_tag:
            ...
            article_url = a_tag['href']
            if not article_url.startswith('http'):
                article_url = 'https://www.sprucepointcap.com' + article_url
            # Get the headline
            title_tag = a_tag.find('h3')
            if not title_tag:
                print("Spruce: Could not find 'h3' tag.")
                time.sleep(3)
                continue
            spruce_headline[1] = title_tag.get_text(strip=True)
            now = datetime.now().time()
